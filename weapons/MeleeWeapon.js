class MeleeWeapon extends Weapon {
    ...
    _getSliceAngles(dx, dy) {
        if (dy === -1) return { startAngle: Math.PI,      sweepAngle:  Math.PI }; // up:    left → top → right
        if (dy ===  1) return { startAngle: Math.PI,      sweepAngle: -Math.PI }; // down:  left → bottom → right
        if (dx ===  1) return { startAngle: Math.PI,      sweepAngle:  Math.PI }; // right: left → top → right
        return            { startAngle: 0,             sweepAngle: -Math.PI };    // left:  right → top → left
    }
    ...
    _drawSlicing(player) {
        const progress = (this.attackDuration - this.attackTimer) / this.attackDuration;
        const { startAngle, sweepAngle } = this._getSliceAngles(this.attackDir.dx, this.attackDir.dy);
